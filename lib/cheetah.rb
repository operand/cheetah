...
class CheetahMailAuthorizationException < CheetahMailException
end

class CheetahMailTemporaryException < CheetahMailException
end

class CheetahMailPermanentException < CheetahMailException
end
...
class CheetahMailer
  ...
  def do_request(path, params)
    ...
    begin
      ...
      @cookie = nil
      retry
    rescue CheetahMailTemporaryException, Timeout::Error => e
      ...
      log_msg = "ERROR: #{e}: #{log_msg}"
      raise e
    rescue CheetahMailPermanentException, Exception => e
  ...
  def do_post(path, params, initheader = nil)
    ...
    raise CheetahMailTemporaryException,     "failure:'#{path}?#{data}', HTTP error: #{resp.code}"              if resp.code =~ /5../
      ...
      raise CheetahMailAuthorizationException, "failure:'#{path}?#{data}', CheetahMail error: #{resp.body.strip}" if resp.body =~ /^err:auth/
      raise CheetahMailTemporaryException,     "failure:'#{path}?#{data}', CheetahMail error: #{resp.body.strip}" if resp.body =~ /^err:internal error/
      raise CheetahMailPermanentException,     "failure:'#{path}?#{data}', CheetahMail error: #{resp.body.strip}" if resp.body =~ /^err/
  ...
  def send_email(eid, email, params = {})
    ...
    params['eid']   = CM_EMAIL_ID_MAP[eid]
    params['email'] = email
    send(path, params, Time.now.usec % 8 + 1)
  end

  def mailing_list_update(subscriber)
    path = "/api/setuser1"
    params          = {}
    ...
    params['sub']   = CM_SUB_ID
    params          = params.merge(map_demographics(subscriber)) if subscriber.class == User
    params          = params.merge(map_city_interests(subscriber))
    send(path, params)
  end
  ...
  # This returns a users information as it appears in CheetahMail's database.
